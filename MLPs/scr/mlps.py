...
class Args():
    ...
    layers_nodes = [64, 32, 1]  # 隐藏层的节点个数，一般在0-1000之间，读者可以自行调整
    learning_rate_base = 1.0  # 训练weights的速率η
    regularzation_rate = 0.001  # 正则力度
    drop_rate = [1.0, 1.0, 1.0]
...
class MLPS(object):
    ...
    def build_model(self):
        ...
        layer_1 = tf.layers.dense(din_all, self.layers_nodes[0], activation=tf.nn.sigmoid, use_bias=True,
        ...
        layer_2 = tf.layers.dense(layer_1, self.layers_nodes[1], activation=tf.nn.sigmoid, use_bias=True,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(self.regularzation_rate),
                                  name='f2')
        layer_2 = tf.nn.dropout(layer_2, keep_prob=self.drop_rate[1])

        layer_3 = tf.layers.dense(layer_2, self.layers_nodes[2], activation=tf.nn.sigmoid, use_bias=True,
                                  kernel_regularizer=tf.contrib.layers.l2_regularizer(self.regularzation_rate),
                                  name='f3')
        self.output = tf.nn.dropout(layer_3, keep_prob=self.drop_rate[2])
        ...
        if args.is_training:
            ...
            for k in range(epoch):
                ...
                for i in range(1, cnt):
                    ...
                    loss, step = Model.train(sess, data['X'][start:end, :], data['Y'][start:end])
                    if i % 10 == 0:
